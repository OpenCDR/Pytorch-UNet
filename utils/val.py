...
from torchseg.utils import metrics, TimeAverager, calculate_eta, logger, progbar
from torchseg.core import infer
...
def evaluate(model,
    ...
    Args:
        ...
        eval_dataset (torch.io.Dataset): Used to read and process validation datasets.
    ...
    """
    model.eval()
    nranks = torch.distributed.ParallelEnv().nranks
    local_rank = torch.distributed.ParallelEnv().local_rank
    if nranks > 1:
        # Initialize parallel environment if not done.
        if not torch.distributed.parallel.parallel_helper._is_parallel_ctx_initialized(
        ):
            torch.distributed.init_parallel_env()
    batch_sampler = torch.io.DistributedBatchSampler(
    ...
    intersect_area_all = torch.zeros([1], dtype='int64')
    pred_area_all = torch.zeros([1], dtype='int64')
    label_area_all = torch.zeros([1], dtype='int64')
    logits_all = None
    label_all = None
