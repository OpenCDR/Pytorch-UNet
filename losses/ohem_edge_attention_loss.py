...
class OhemEdgeAttentionLoss(nn.Module):
    """
    ...
    def forward(self, logits, label):
        ...
        # Filter out edge
        filler = torch.ones_like(label) * self.ignore_index
        label = torch.where(edge_logit > self.edge_threshold, label, filler)
        ...
        loss = F.cross_entropy(
            seg_logit, label, ignore_index=self.ignore_index)
...
if __name__ == '__main__':
    ...
    mask=torch.randn(batch_size,C,320,320)
    ohem=OhemEdgeAttentionLoss()
